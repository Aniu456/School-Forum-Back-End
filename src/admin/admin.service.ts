...
export class AdminService {
   ...
   */
  private async findUserOrThrow(userId: string, select?: Record<string, any>) {
  ...
  async unhighlightPost(postId: string) {
    ...
    return { message: '已取消加精', post: updatedPost };
  }
  ...
  async bulkDeleteComments(ids: string[]) {
            ...
            .then((post) => {
              if (post && post.commentCount > 0) {
                const newCount = Math.max(
                  0,
                  post.commentCount - decrementCount,
                );
    ...
    const totalDeleted = Array.from(postCommentCounts.values()).reduce(
      (sum, count) => sum + count,
      0,
    );
