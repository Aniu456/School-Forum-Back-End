# 📚 校园论坛后台系统

> **NestJS + PostgreSQL + Redis + 阿里云**  
> 一个功能完整的校园社交论坛后端系统

---

## 📋 目录

- [项目简介](#项目简介)
- [核心功能](#核心功能)
- [技术栈](#技术栈)
- [系统架构](#系统架构)
- [快速开始](#快速开始)
- [项目结构](#项目结构)
- [数据库设计](#数据库设计)
- [文档导航](#文档导航)

---

## 项目简介

校园论坛后台系统是基于 **NestJS** 框架开发的 RESTful API 服务，为校园用户提供发帖、评论、点赞、搜索等社交互动功能。系统采用模块化架构设计，支持水平扩展和微服务化演进。

### 特点

- ✅ **模块化设计**：8+ 核心模块，职责清晰
- ✅ **类型安全**：TypeScript + Prisma 全栈类型安全
- ✅ **高性能**：Redis 缓存 + 数据库索引优化
- ✅ **安全可靠**：JWT 认证 + 角色权限控制
- ✅ **云原生**：支持阿里云 ECS/RDS/Redis/OSS 部署
- ✅ **实时通知**：WebSocket 实时推送
- ✅ **完整文档**：API 文档 + 开发指南 + 部署指南

---

## 核心功能

### 用户功能
- 用户注册/登录（JWT 认证）
- 个人资料管理
- 关注/粉丝系统
- 个性化推荐

### 内容功能
- 帖子发布/编辑/删除
- 评论与二级回复
- 点赞/收藏
- 草稿自动保存
- 图片上传（阿里云 OSS）

### 社交功能
- 全文搜索
- 热门/趋势推荐
- 话题标签
- 实时通知

### 管理功能
- 用户管理（封禁/解封）
- 内容审核
- 举报处理
- 数据统计

---

## 技术栈

### 后端框架
- **NestJS** v11 - 渐进式 Node.js 框架
- **TypeScript** v5.7 - 类型安全
- **Prisma** - 现代化 ORM

### 数据库
- **PostgreSQL** 14+ - 主数据库
- **Redis** 6.0+ - 缓存与 Session

### 认证与安全
- **JWT** - 无状态认证
- **Passport** - 认证中间件
- **bcrypt** - 密码加密

### 云服务（阿里云）
- **ECS** - 应用服务器
- **RDS PostgreSQL** - 托管数据库
- **Redis** - 托管缓存
- **OSS** - 对象存储
- **CDN** - 内容分发

---

## 系统架构

```
┌─────────────────────────────────────────────────────────┐
│                     Client Layer                         │
│              (Web / Mobile / Desktop)                    │
└────────────────────┬────────────────────────────────────┘
                     │ HTTP/HTTPS
                     │
┌────────────────────▼────────────────────────────────────┐
│              Application Layer (NestJS)                  │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐             │
│  │  Auth    │  │  Posts   │  │  Users   │             │
│  │  Module  │  │  Module  │  │  Module  │             │
│  └──────────┘  └──────────┘  └──────────┘             │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐             │
│  │ Comments │  │  Likes   │  │  Search  │             │
│  │  Module  │  │  Module  │  │  Module  │             │
│  └──────────┘  └──────────┘  └──────────┘             │
└────────────────────┬────────────────────────────────────┘
                     │
        ┌────────────┼────────────┐
        │            │            │
┌───────▼──────┐ ┌──▼──────┐ ┌──▼────────┐
│ PostgreSQL   │ │  Redis  │ │ Ali OSS   │
│   (RDS)      │ │ (Cache) │ │  (File)   │
└──────────────┘ └─────────┘ └───────────┘
```

### 核心模块

| 模块 | 职责 | 依赖 |
|-----|------|------|
| `auth` | 用户认证、注册、登录 | `prisma`, `users` |
| `users` | 用户信息管理 | `prisma` |
| `posts` | 帖子 CRUD | `prisma`, `users` |
| `comments` | 评论功能 | `prisma`, `posts` |
| `likes` | 点赞功能 | `prisma` |
| `notifications` | 通知推送 | `prisma`, `users` |
| `admin` | 管理员功能 | `prisma` |
| `search` | 全文搜索 | `prisma` |

---

## 快速开始

### 1. 环境要求

```bash
node -v    # >= 18.0.0
pnpm -v    # >= 8.0.0
psql --version  # PostgreSQL >= 14
redis-cli --version  # Redis >= 6.0
```

### 2. 克隆项目

```bash
git clone <repository-url>
cd school-forum-back-end
```

### 3. 安装依赖

```bash
pnpm install
```

### 4. 配置环境变量

```bash
cp .env.example .env
vim .env
```

**关键配置**：
```env
# 数据库
DATABASE_URL="postgresql://user:password@localhost:5432/school_forum"

# JWT
JWT_SECRET=your-secret-key-min-32-chars
JWT_REFRESH_SECRET=your-refresh-secret-key

# Redis
REDIS_HOST=127.0.0.1
REDIS_PORT=6379

# CORS
CORS_ORIGIN=http://localhost:3000,http://localhost:3001,http://localhost:5173,http://localhost:5174
```

### 5. 初始化数据库

```bash
# 生成 Prisma Client
pnpm prisma generate

# 运行数据库迁移
pnpm prisma migrate dev

# (可选) 填充测试数据
pnpm prisma db seed
```

### 6. 启动开发服务器

```bash
pnpm run start:dev
```

应用将在 `http://localhost:3000` 启动。

### 7. 验证安装

```bash
# 测试健康检查
curl http://localhost:3000

# 查看 Prisma Studio
pnpm prisma studio
```

---

## 项目结构

```
src/
├── auth/                       # 认证模块
├── users/                      # 用户模块
├── posts/                      # 帖子模块
├── comments/                   # 评论模块
├── likes/                      # 点赞模块
├── notifications/              # 通知模块
├── search/                     # 搜索模块
├── admin/                      # 管理模块
├── favorites/                  # 收藏模块
├── follows/                    # 关注模块
├── drafts/                     # 草稿模块
├── recommendations/            # 推荐模块
│
├── common/                     # 公共模块
│   ├── decorators/             # 装饰器
│   ├── guards/                 # 守卫
│   ├── filters/                # 异常过滤器
│   ├── interceptors/           # 拦截器
│   └── middleware/             # 中间件
│
├── prisma/                     # Prisma 服务
├── redis/                      # Redis 服务
├── app.module.ts               # 根模块
└── main.ts                     # 应用入口
```

---

## 数据库设计

### 核心表结构

#### users (用户表)
- id (PK, UUID)
- username (唯一索引)
- email (唯一索引)
- password (加密)
- role (STUDENT/TEACHER/ADMIN)
- created_at, updated_at

#### posts (帖子表)
- id (PK, UUID)
- title (全文索引)
- content (全文索引)
- author_id (FK → users.id)
- tags (数组)
- view_count, like_count, comment_count
- created_at, updated_at

#### comments (评论表)
- id (PK, UUID)
- content
- post_id (FK → posts.id)
- author_id (FK → users.id)
- parent_id (FK → comments.id, 可空)
- created_at, updated_at

#### likes (点赞表)
- id (PK, UUID)
- user_id (FK → users.id)
- target_id
- target_type (POST/COMMENT)
- UNIQUE(user_id, target_id, target_type)

### 扩展功能表
- **favorites** - 收藏
- **follows** - 关注
- **post_drafts** - 草稿
- **notifications** - 通知
- **reports** - 举报

---

## 文档导航

### 📖 核心文档（必读）

1. **[DEVELOPMENT.md](./DEVELOPMENT.md)** - 开发指南
   - 开发环境搭建
   - API 接口文档
   - 数据库使用指南
   - 代码规范与最佳实践

2. **[DEPLOYMENT.md](./DEPLOYMENT.md)** - 部署指南
   - 阿里云资源配置
   - 服务器环境搭建
   - 应用部署流程
   - 故障排查与运维

### 📁 前端对接文档

- **[FRONTEND_USER_HANDOVER.md](./FRONTEND_USER_HANDOVER.md)** - 用户端前端交接
- **[FRONTEND_ADMIN_HANDOVER.md](./FRONTEND_ADMIN_HANDOVER.md)** - 管理端前端交接

### 🔗 快速链接

- **开发环境搭建** → [DEVELOPMENT.md#快速开始](./DEVELOPMENT.md)
- **API 接口文档** → [DEVELOPMENT.md#API文档](./DEVELOPMENT.md)
- **数据库迁移** → [DEVELOPMENT.md#数据库使用](./DEVELOPMENT.md)
- **部署到阿里云** → [DEPLOYMENT.md#部署流程](./DEPLOYMENT.md)
- **常见问题排查** → [DEPLOYMENT.md#故障排查](./DEPLOYMENT.md)

---

## 开发团队

- **项目负责人**: 后端开发团队
- **技术支持**: support@example.com
- **问题反馈**: GitHub Issues
- **最后更新**: 2025-11-16

---

<div align="center">

**🚀 开始开发**: 阅读 [DEVELOPMENT.md](./DEVELOPMENT.md)  
**☁️ 部署上线**: 阅读 [DEPLOYMENT.md](./DEPLOYMENT.md)

Made with ❤️ by 后端开发团队

</div>
